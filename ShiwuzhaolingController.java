...
//            .notIn("shiwuzhaoling_types", new Integer[]{102})
    ...
    public R groupByType(HttpServletRequest request){
        logger.debug("groupByType方法:,,Controller:{}", this.getClass().getName());

        // 调用Service层获取数据
        List<Map<String, Object>> result = shiwuzhaolingService.groupByType();

        // 记录操作日志
        caozuorizhiService.insertCaozuorizhi(
...
//     * 月度认领统计
//     */
//    @GetMapping("/monthlyClaimStats")
//    public R monthlyClaimStats(
//            @RequestParam(required = false) String month,
//            HttpServletRequest request
//    ){
//        logger.debug("monthlyClaimStats方法:,,Controller:{},month:{}", this.getClass().getName(), month);
//
//        // 调用Service层获取数据
//        List<Map<String, Object>> result = shiwuzhaolingService.monthlyClaimStats(month);
//
//        // 记录操作日志
//        caozuorizhiService.insertCaozuorizhi(
//                String.valueOf(request.getSession().getAttribute("role")),
...
//    }
    ...
    public R groupByStatus(HttpServletRequest request){
        logger.debug("groupByStatus方法:,,Controller:{}", this.getClass().getName());

        // 调用Service层获取数据
        List<Map<String, Object>> result = shiwuzhaolingService.groupByStatus();
        // 记录操作日志
        caozuorizhiService.insertCaozuorizhi(
                String.valueOf(request.getSession().getAttribute("role")),
                TABLE_NAME,// 表名常量
...
// 返回统一响应格式 R，标记为成功（ok()），并将统计结果放入 "data" 字段
        return R.ok().put("data", result);
    }
